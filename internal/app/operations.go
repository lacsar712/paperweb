...
func (a *App) StartDewater(ctx context.Context, holder string) error {
	now := a.clk.Now()
	return a.interlock.Leases().WithLease(ctx, a.cfg.UnitID, holder, now, func(ctx context.Context) error {
		...
		comb, err := a.headbox.StartDewater(ctx, snap)
		if err != nil {
			return err
		}
		_ = a.store.UpdateHeadbox(a.cfg.UnitID, comb)
		state, err := a.fsm.Dispatch(ctx, fsm.EvStartDewater)
		if err != nil {
			return err
		}
		a.syncState(state)
		a.journalEvent("start_dewater", storePayload("holder", holder))
		return nil
	})
}

func (a *App) CompleteDewater(ctx context.Context, holder string) error {
	now := a.clk.Now()
	return a.interlock.Leases().WithLease(ctx, a.cfg.UnitID, holder, now, func(ctx context.Context) error {
		snap := a.Snapshot()
		if err := a.headbox.CompleteDewater(snap.Headbox); err != nil {
			return err
		}
		state, err := a.fsm.Dispatch(ctx, fsm.EvDewaterComplete)
		if err != nil {
			return err
		}
		a.syncState(state)
		a.journalEvent("dewater_complete", "")
		return nil
	})
}
...
func (a *App) Ignite(ctx context.Context, holder string) error {
	...
	return a.interlock.Leases().WithLease(ctx, a.cfg.UnitID, holder, now, func(ctx context.Context) error {
		...
		comb, err := a.headbox.Ignite(ctx, snap)
		if err != nil {
			return err
		}
		_ = a.store.UpdateHeadbox(a.cfg.UnitID, comb)
...
func (a *App) StabilizeIgnition(ctx context.Context, holder string) error {
	...
	return a.interlock.Leases().WithLease(ctx, a.cfg.UnitID, holder, now, func(ctx context.Context) error {
		snap := a.Snapshot()
		comb, err := a.headbox.Stabilize(snap)
		if err != nil {
			return err
		}
		_ = a.store.UpdateHeadbox(a.cfg.UnitID, comb)
...
func (a *App) RampLoad(ctx context.Context, holder string, loadPct float64) error {
	...
	return a.interlock.Leases().WithLease(ctx, a.cfg.UnitID, holder, now, func(ctx context.Context) error {
		...
		comb := a.headbox.RampToLoad(snap, loadPct)
		_ = a.store.UpdateHeadbox(a.cfg.UnitID, comb)
		wirelineReading, err := a.wireline.Tick(ctx, snap, true)
		if err != nil {
			return err
		}
		_ = a.store.UpdateWireline(a.cfg.UnitID, wirelineReading)
...
func (a *App) UpdateSettings(ctx context.Context, settings model.PlantSettings) error {
	...
	if err := a.wireline.ValidateSettings(settings); err != nil {
		return err
	}
...
func (a *App) OnSheetLoss(ctx context.Context, holder string) error {
	_ = holder
	snap := a.Snapshot()
	if err := interlock.CheckSheetLoss(snap.Headbox); err != nil {
		return fmt.Errorf("sheet loss: %w", err)
	}
	return nil
}
...
func (a *App) runTick(ctx context.Context) error {
	...
	couchReading, err := a.couch.Tick(ctx, snap, firing)
	if err != nil {
		return err
	}
	_ = a.store.UpdateCouch(a.cfg.UnitID, couchReading)
	snap, _ = a.store.Require(a.cfg.UnitID)
	wirelineReading, err := a.wireline.Tick(ctx, snap, firing)
	if err != nil {
		return err
	}
	_ = a.store.UpdateWireline(a.cfg.UnitID, wirelineReading)
