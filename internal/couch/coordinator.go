...
func NewCoordinator(clk clock.ProcessClock) *Coordinator {
	return &Coordinator{
		...
		settle:    clock.NewCouchSwellWindow(clk),
	}
}
...
func (c *Coordinator) Tick(ctx context.Context, snap model.PlantSnapshot, firing bool) (model.CouchReading, error) {
	select {
	case <-ctx.Done():
		return snap.Couch, fmt.Errorf("%w", model.ErrContextDone)
	default:
	}
	out := snap.Couch
	...
	if firing {
		out.SteamFlowTPH = snap.Wireline.MainSteamFlowTPH
	}
	out.CarryoverPPM = c.carryover.Estimate(out, snap.Wireline.SteamPressurePSI)
	if cond == model.CouchSwell {
		out.LastSwellAt = c.clk.Now()
	}
	return out, nil
}

func (c *Coordinator) SettledAfterSwell(snap model.CouchReading) bool {
...
func (c *Coordinator) RequireSettled(snap model.CouchReading) error {
	if snap.LastSwellAt.IsZero() {
		return nil
	}
...
func (c *Coordinator) TripRequired(snap model.CouchReading) bool {
	return snap.LevelPercent < model.TripCouchLowPercent || snap.LevelPercent > model.TripCouchHighPercent
}
