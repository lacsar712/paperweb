...
func (s *SwellDetector) Observe(level float64) (bool, model.CouchCondition) {
	delta := level - s.lastLevel
	s.lastLevel = level
	if delta > 5 {
		s.lastChangeAt = s.clk.Now()
		return true, model.CouchSwell
	}
	if delta < -5 {
		s.lastChangeAt = s.clk.Now()
		return true, model.CouchShrink
	}
	return false, model.CouchNormal
}
...
func (s *SwellDetector) ShrinkRecoveryAdvice(cond model.CouchCondition) string {
	if cond == model.CouchShrink {
