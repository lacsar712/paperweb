...
func (c *Controller) Tick(ctx context.Context, snap model.PlantSnapshot, firing bool) (model.WirelineReading, error) {
	select {
	case <-ctx.Done():
		return snap.Wireline, fmt.Errorf("%w", model.ErrContextDone)
	default:
	}
	pressure, err := c.pressure.Compute(snap, firing)
	if err != nil {
		return snap.Wireline, err
	}
