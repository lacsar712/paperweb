...
func (p *PressureModel) Compute(snap model.PlantSnapshot, firing bool) (float64, error) {
	if !firing {
		return p.cooldown(snap.Wireline.SteamPressurePSI), nil
	}
	base := snap.Settings.TargetSteamPSI
	fiberFactor := snap.Headbox.FiberFlowTPH / math.Max(snap.Settings.FiberFlowTPH, 1)
	couchFactor := snap.Couch.LevelPercent / snap.Settings.CouchLevelSetpoint
	if couchFactor <= 0 {
		couchFactor = 0.5
	}
	pressure := base * fiberFactor * math.Min(couchFactor, 1.2)
...
func (p *PressureModel) DeviationFromSetpoint(snap model.PlantSnapshot) float64 {
	return snap.Wireline.SteamPressurePSI - snap.Settings.TargetSteamPSI
}
