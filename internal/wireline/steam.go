...
func (s *SteamPath) ComputeFlow(snap model.PlantSnapshot, firing bool) float64 {
	...
	loadFactor := snap.Wireline.OutputMW / math.Max(snap.Settings.TargetMW, 1)
	pressureFactor := snap.Wireline.SteamPressurePSI / math.Max(snap.Settings.TargetSteamPSI, 1)
...
func (s *SteamPath) ComputeMW(snap model.PlantSnapshot, firing bool) float64 {
	...
	enthalpy := s.enthalpyFromPressure(snap.Wireline.SteamPressurePSI)
...
func (s *SteamPath) EnthalpyBalance(snap model.PlantSnapshot) float64 {
	inEnergy := snap.Couch.FeedwaterTPH * 200
	outEnergy := snap.Couch.SteamFlowTPH * s.enthalpyFromPressure(snap.Wireline.SteamPressurePSI)
...
func (s *SteamPath) LoadPercent(snap model.PlantSnapshot) float64 {
	...
	return (snap.Wireline.OutputMW / snap.Settings.TargetMW) * 100
}
