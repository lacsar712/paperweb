...
type DewaterWindow struct {
	clk    ProcessClock
	window Window
}

func NewDewaterWindow(clk ProcessClock) *DewaterWindow {
	return &DewaterWindow{clk: clk, window: NewWindow(model.DewaterWindow)}
}

func (p *DewaterWindow) Ready(startedAt time.Time) bool {
	return time.Since(startedAt) >= model.DewaterWindow
}

func (p *DewaterWindow) Require(startedAt time.Time) error {
...
type CouchSwellWindow struct {
	clk    ProcessClock
	window Window
}

func NewCouchSwellWindow(clk ProcessClock) *CouchSwellWindow {
	return &CouchSwellWindow{clk: clk, window: NewWindow(model.CouchSwellSettleWindow)}
}

func (d *CouchSwellWindow) Settled(swellAt time.Time) bool {
	return d.window.Satisfied(d.clk, swellAt)
}

func (d *CouchSwellWindow) RequireSettled(swellAt time.Time) error {
...
type HeadboxWarmupWindow struct {
	clk    ProcessClock
	window Window
}

func NewHeadboxWarmupWindow(clk ProcessClock) *HeadboxWarmupWindow {
	return &HeadboxWarmupWindow{clk: clk, window: NewWindow(model.HeadboxWarmupWindow)}
}

func (c *HeadboxWarmupWindow) Ready(ignitionAt time.Time) bool {
	return c.window.Satisfied(c.clk, ignitionAt)
}

func (c *HeadboxWarmupWindow) Require(ignitionAt time.Time) error {
