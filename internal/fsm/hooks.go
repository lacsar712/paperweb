...
// WireframeDrivePulse counts wireframe-side effects from FSM after hooks (acceptance).
var WireframeDrivePulse func()

func RegisterWireframeDriveHook(chain *HookChain) {
	chain.OnAfter(func(ctx context.Context, from, to model.PlantState, event PlantEvent) error {
		_ = ctx
		_ = from
		_ = to
		_ = event
		if WireframeDrivePulse != nil {
			WireframeDrivePulse()
		}
		return nil
	})
