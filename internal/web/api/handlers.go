...
func (s *Server) Routes() http.Handler {
	...
	mux.HandleFunc("POST /dewater/start", withTimeout(s.handleStartDewater))
	mux.HandleFunc("POST /dewater/complete", withTimeout(s.handleCompleteDewater))
	...
	mux.HandleFunc("GET /couch/level", withTimeout(s.handleCouchLevel))
...
func (s *Server) handleWarmup(w http.ResponseWriter, r *http.Request) {
	...
	writeOK(w, map[string]interface{}{
		"ready":  ready,
		"detail": detail,
		"dewater":  s.app.DewaterRemaining(),
		"warmup": s.app.HeadboxWarmupRemaining(),
	})
}

func (s *Server) handleStartDewater(w http.ResponseWriter, r *http.Request) {
	holder := r.URL.Query().Get("holder")
	if holder == "" {
		holder = "hmi-operator"
	}
	if err := s.app.StartDewater(r.Context(), holder); err != nil {
...
func (s *Server) handleCompleteDewater(w http.ResponseWriter, r *http.Request) {
	holder := r.URL.Query().Get("holder")
	if holder == "" {
		holder = "hmi-operator"
	}
	if err := s.app.CompleteDewater(r.Context(), holder); err != nil {
		writeErr(w, http.StatusConflict, err)
		return
	}
	writeOK(w, s.app.Snapshot())
...
func (s *Server) handleCouchLevel(w http.ResponseWriter, r *http.Request) {
	if err := s.app.CheckCouchLevel(s.app.Snapshot()); err != nil {
		writeErr(w, http.StatusConflict, fmt.Errorf("wireline fault: %w", err))
		return
	}
